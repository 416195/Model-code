...
n=NT/T;
A1=zeros(1,NT+m+s1+s2+1);b1=0;
LB1=[zeros(NT+m+s1+s2,1);1e-10];UB=[];
LB2=[zeros(NT+m+s1+s2-1,1);1e-10];
w=[];fval=[];
...
if rts==0;
    for t=1:T;
        for i=1:n;
            f1=[zeros(1,NT) -(1/m)*DX(i+(t-1)*n,:) zeros(1,s1) zeros(1,s2) 1];
            Aeq1=[X(:,1:NT) eye(m) zeros(m,s1) zeros(m,s2) -X(:,i+(t-1)*n);
                Y(:,1:NT) zeros(s1,m) -eye(s1) zeros(s1,s2) -Y(:,i+(t-1)*n);
                Z(:,1:NT) zeros(s2,m) zeros(s2,s1) eye(s2) -Z(:,i+(t-1)*n);
                zeros(1,NT) zeros(1,m) (1/(s1+s2))*DY(i+(t-1)*n,:) (1/(s1+s2))*DZ(i+(t-1)*n,:) 1];
            beq1=[zeros(m,1);
                zeros(s1,1);
                zeros(s2,1);
                1];
            try
                ...
                w(:,i+(t-1)*n)=nan(NT+m+s1+s2+1,1);
                fval1(:,i+(t-1)*n)=nan;
            end
            if Super==0;
                fval(:,i+(t-1)*n)=fval1(:,i+(t-1)*n);
            elseif Super==1;
                f2=[zeros(1,NT-1) (1/m)*DX(i+(t-1)*n,:) zeros(1,s1) zeros(1,s2) 1];
                Aeq2=[zeros(1,NT-1) zeros(1,m) -(1/(s1+s2))*DY(i+(t-1)*n,:) -(1/(s1+s2))*DZ(i+(t-1)*n,:) 1];
                beq2=1;
                X2=X(:,1:NT);Y2=Y(:,1:NT);Z2=Z(:,1:NT);
                X2(:,i+(t-1)*n)=[];Y2(:,i+(t-1)*n)=[];Z2(:,i+(t-1)*n)=[];
