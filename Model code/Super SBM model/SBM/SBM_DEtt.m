function [lambda,s_x,s_y,E_tt,E_t1t1]=SBM_DEtt(data,T,xs,ys,rts,Super);
...
LB1=[zeros(n+m+s1,1);1e-10];UB=[];
LB2=[zeros(n+m+s1-1,1);1e-10];
w=[];fval=[];
...
if rts==0;
    for t=1:T
        for i=1:n
            f1=[zeros(1,n) -(1/m)*DX(i+(t-1)*n,:) zeros(1,s1) 1];
            Aeq1=[X(:,n*(t-1)+1:n*t) eye(m) zeros(m,s1) -X(:,i+(t-1)*n);
                Y(:,n*(t-1)+1:n*t) zeros(s1,m) -eye(s1) -Y(:,i+(t-1)*n);
                zeros(1,n) zeros(1,m) (1/s1)*DY(i+(t-1)*n,:) 1];
            beq1=[zeros(m,1);
                zeros(s1,1);
                1];
            try
            ...
            elseif Super==1;
                f2=[zeros(1,n-1) (1/m)*DX(i+(t-1)*n,:) zeros(1,s1) 1];
                Aeq2=[zeros(1,n-1) zeros(1,m) -(1/s1)*DY(i+(t-1)*n,:) 1];
                beq2=1;
                X2=X(:,n*(t-1)+1:n*t);Y2=Y(:,n*(t-1)+1:n*t);
                X2(:,i)=[];Y2(:,i)=[];
