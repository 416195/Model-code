function eff = SBM_UNt1t(data,T,xs,ys,yb,rts);
...
if rts==0;
    for t=1:(T-1);
        for i=1:n;
            ...
            Aeq1=[X(:,n*t+1:n*(t+1)) eye(m) zeros(m,s1) zeros(m,s2) -X(:,i+(t-1)*n);
                Y(:,n*t+1:n*(t+1)) zeros(s1,m) -eye(s1) zeros(s1,s2) -Y(:,i+(t-1)*n);
                Z(:,n*t+1:n*(t+1)) zeros(s2,m) zeros(s2,s1) eye(s2) -Z(:,i+(t-1)*n);
                zeros(1,n) zeros(1,m) (1/(s1+s2))*DY(i+(t-1)*n,:) (1/(s1+s2))*DZ(i+(t-1)*n,:) 1];
            beq1=[zeros(m,1);
                zeros(s1,1);
                zeros(s2,1);
                1];
            f2=[zeros(1,n) (1/m)*DX(i+(t-1)*n,:) zeros(1,s1) zeros(1,s2) 1];
            Aeq2=[zeros(1,n) zeros(1,m) -(1/(s1+s2))*DY(i+(t-1)*n,:) -(1/(s1+s2))*DZ(i+(t-1)*n,:) 1];
            beq2=1;
            A2=[X(:,n*t+1:n*(t+1)) -eye(m) zeros(m,s1) zeros(m,s2) -X(:,i+(t-1)*n);
                -Y(:,n*t+1:n*(t+1)) zeros(s1,m) -eye(s1) zeros(s1,s2) Y(:,i+(t-1)*n);
                Z(:,n*t+1:n*(t+1)) zeros(s2,m) zeros(s2,s1) -eye(s2) -Z(:,i+(t-1)*n)];
            b2=[zeros(m,1);
                zeros(s1,1);
